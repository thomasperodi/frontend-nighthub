  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }] }>
      <View style={{ padding: 18 }}>
        <Text style={{ fontSize: 18, fontWeight: '800', color: theme.colors.text }}>{res.eventTitle}</Text>
        <Text style={{ color: theme.colors.muted, marginTop: 6 }}>{res.zoneName} • {res.tableName}</Text>
        ...
        <View style={[{ padding: 14, borderRadius: 12, borderWidth: 1, borderColor: theme.colors.border }] }>
          ...
          {res.seats ? (
            <Text style={{ color: theme.colors.muted, marginTop: 6 }}>{res.seats} persone</Text>
          ) : null}
          ...
          {res.promoAppliedId ? (
            <View style={{ marginTop: 8 }}>
              <Text style={{ color: theme.colors.muted }}>Promozione applicata</Text>
              <Text style={{ color: theme.colors.text, fontWeight: '700' }}>{(MOCK_PROMOS.find(x=>x.id===res.promoAppliedId) || MOCK_EVENTS.flatMap(e=>e.promos||[]).find(p=>p.id===res.promoAppliedId))?.title || res.promoAppliedId}</Text>
            </View>
          ) : null}
